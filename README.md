...
├── Dockerfile                      # Cloud Run 배포용 컨테이너 설정
├── .dockerignore                   # Docker 빌드 제외 목록
├── DEPLOY_GUIDE.txt                # Cloud Run 배포 가이드
...
| **알림/점검** | 알림 발송, 서버 점검 예약 (KST 카운트다운) |
...
```bash
# 1) Google Cloud CLI 설치 후 초기화
gcloud init

# 2) API 활성화
gcloud services enable run.googleapis.com cloudbuild.googleapis.com artifactregistry.googleapis.com

# 3) 배포 (Dockerfile 자동 인식)
gcloud run deploy testbase \
  --source . \
...
# 4) 환경변수 설정 (시크릿)
gcloud run services update testbase \
  --region asia-northeast3 \
  --set-env-vars "TURSO_DATABASE_URL=..." \
  --set-env-vars "TURSO_AUTH_TOKEN=..." \
  --set-env-vars "KEY_POOL_JSON=..."
```
