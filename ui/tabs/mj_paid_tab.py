# ui/tabs/mj_paid_tab.py
"""Midjourney Paid 탭 — MJ 탭과 동일 기능, Fast/Turbo 모드 사용."""
...
from providers import useapi_mj
from ui.sidebar import SidebarState

# 전용 HTML (Fast/Turbo 활성화)
_COMPONENT_DIR = Path(__file__).resolve().parent / "templates" / "mj_paid"
_mj_paid_component_func = components.declare_component("mj_paid_component", path=str(_COMPONENT_DIR))

# ── 세션 키 (mj/mj_free 탭과 분리) ──
_PREFIX = "mjp"
...
def _mj_paid_component(gallery_items, frame_height=900, key="mj_paid_main",
                        enabled_features=None, school_gallery=None,
                        source_gallery=None, default_model=""):
    return _mj_paid_component_func(
...
def _build_mj_paid_full_text(prompt: str, settings: dict) -> str:
    """Fast/Turbo 모드 프롬프트 조합."""
    parts = [prompt]
...
def render_mj_paid_tab(cfg: AppConfig, sidebar: SidebarState):
    """MJ Paid 탭 (Fast/Turbo 모드)."""
    ...
    if pending:
        ...
        try:
            mj_prompt = _build_mj_paid_full_text(pending["prompt"], pending.get("settings", {}))
        ...
        if image_urls:
            ...
            try:
                _speed = pending.get("settings", {}).get("speed", "Fast")
                _multiplier = 2 if _speed == "Turbo" else 1
                _cost = get_feature_cost(cfg, "mj") * _multiplier
                new_bal = deduct_after_success(cfg, _cost, tab_id="mj_paid")
    ...
    if describe_pending:
        ...
        try:
            ...
            from core.credits import deduct_after_success, get_feature_cost
            try:
                new_bal = deduct_after_success(cfg, get_feature_cost(cfg, "mj_describe"), tab_id="mj_describe")
                if new_bal is not None:
                    st.session_state[_K_CREDIT] = new_bal
            except Exception:
                pass
    ...
    if _err:
        ...
        insert_error_log(cfg, st.session_state.get("user_id", ""), st.session_state.get("school_id", "default"), "mj_paid", _err)
    ...
    if action == "open_gallery":
        ...
        ok, msg = check_credits(cfg, get_feature_cost(cfg, "mj_describe"))
        if not ok:
        ...
        # ── 크레딧 확인 (Phase 1, Turbo면 2배) ──
        _speed = s.get("speed", "Fast")
        _multiplier = 2 if _speed == "Turbo" else 1
        from core.credits import check_credits, get_feature_cost
        _cost = get_feature_cost(cfg, "mj") * _multiplier
        ...
        tags.append(_speed.lower())  # fast 또는 turbo
...
TAB = {
    "tab_id": "mj_paid",
    "title": "Image Create",
    "required_features": {"tab.mj_paid"},
    "render": render_mj_paid_tab,
}
